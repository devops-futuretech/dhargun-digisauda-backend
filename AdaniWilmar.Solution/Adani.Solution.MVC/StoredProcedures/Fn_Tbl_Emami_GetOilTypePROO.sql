IF EXISTS (SELECT * FROM sysobjects WHERE type IN (N'FN', N'IF', N'TF') AND name = 'Fn_Tbl_Emami_GetOilTypePROO')
    BEGIN
        DROP  Function Fn_Tbl_Emami_GetOilTypePROO
    END
 Go
SET ANSI_NULLS ON
GO
...
GO
CREATE FUNCTION [dbo].[Fn_Tbl_Emami_GetOilTypePROO] 
(  
   @PlantId BigInt,  
   @StateId BigInt,  
   @OilTypeId BigInt
...
as  
	...
	return Select 
		...
		Sum(Case 
			...
			+ Case When SO.SpecialRateRequestId > 0 and SO.BidQuantityCase > 0
			  ...
			  End) ---Premium
			- Case 
				When SO.SpecialRateRequestId > 0 
				Then (SO.QuotedPrice - SO.BidPrice) / SO.BidQuantityCase 
				Else 
			  Case 
				When SO.DiscountTypeId = 2 and SO.BidQuantityCase > 0
			...
			- Case 
				...
				Else 
					Case 
						...
						End 
					End --- Discount
		End  - 0 / (Case When SKU.UomId = 2 Then (SKU.Quantity * 1000) / OT.LitreConversion Else SKU.Quantity End) * 1000 + 0) ) As RealizationPMT
	...
	Where SO.PlantId = @PlantId And D.StateId = @StateId And SO.OilTypeId = @OilTypeId
	Group By
