...
/****** Object:  StoredProcedure [dbo].[SetUserCustomerSaudaTarget]    Script Date: 10-06-2022 12:01:33 ******/
SET ANSI_NULLS ON
GO
...
AS
DECLARE @AssignedFromId bigint, @AssignedToId bigint, @TargetId bigint, @MonthId bigint,@FinancialYearId bigint,@DivisionId bigint, @OilTypeId bigint,@UserCustomerSaudaTargetId bigint
...
--AssignedFrom begins
IF NOT EXISTS(Select 1 FROM [Users] u,UserDivisionMappings ud Where [Code]= @AssignedFromUserCode and ud.DivisionId=@DivisionId and u.Id= ud.UserId and [IsActive] =  1)
BEGIN
	ROLLBACK
...
END
ELSE
	Select @AssignedFromId = u.Id From [Users] u,UserDivisionMappings ud Where [Code]= @AssignedFromUserCode and ud.DivisionId=@DivisionId and ud.UserId=u.Id and [IsActive] =  1
...
--AssignedTo begins
IF NOT EXISTS(Select 1 FROM [Users] u,UserDivisionMappings ud Where [Code]= @AssignedToUserCode and ud.DivisionId=@DivisionId and ud.UserId=u.Id and [IsActive] =  1)
BEGIN
	ROLLBACK
...
END
ELSE
	Select @AssignedToId = u.Id From Users u,UserDivisionMappings ud Where [Code]= @AssignedToUserCode and ud.DivisionId=@DivisionId and u.Id=ud.UserId and [IsActive] =  1
...
IF NOT EXISTS(Select 1 FROM [UserCustomerSaudaTargets] Where [AssignedFromId]= @AssignedFromId and [AssignedToId]=@AssignedToId and [DivisionId]=@DivisionId and [OilTypeId]=@OilTypeId and [MonthId]=@MonthId and [FinancialYearId]=@FinancialYearId and [Year]=@Year)
BEGIN	
	...
	BEGIN
		INSERT INTO [dbo].[UserCustomerSaudaTargets]([AssignedFromId],[AssignedToId],[Quarter],[MonthId],[FinancialYearId],[Year],[DivisionId],[OilTypeId],[Target],[CreatedBy],[CreatedDate])
	...
	ELSE
		BEGIN
			IF EXISTS (SELECT Top 1 1 FROM [UserCustomerSaudaTargets] WHERE [AssignedToId]=@AssignedFromId and [FinancialYearId]=@FinancialYearId and [DivisionId]=@DivisionId and OilTypeId = @OilTypeId
...
ELSE
	...
	SELECT @UserCustomerSaudaTargetId = Id FROM [UserCustomerSaudaTargets] 
		...
		IF (@UserCustomerSaudaTargetId>0)
		BEGIN
			UPDATE [dbo].[UserCustomerSaudaTargets]
			 SET [Target] =@Target,[ModifiedBy] = @CreatedBy,[ModifiedDate] = getdate()
			Where Id=@UserCustomerSaudaTargetId
		END
		ELSE
		BEGIN
			SELECT @SalesOrganizationCode as SalesOrganizationCode,@DistributionChannelCode as DistributionChannelCode,@AssignedFromUserCode as AssignedFromUserCode,@AssignedToUserCode as AssignedToUserCode,@Month as 'Month',@FinancialYear as FinancialYear,@Quarter as 'Quarter',@DivisionCode as DivisionCode,@OilTypeName as OilTypeName,@Year as 'Year',@Target as 'Target', 'Failed in UserCustomerSaudaTarget' as 'Message'  FROM [UserCustomerSaudaTargets] Where [AssignedFromId]= @AssignedFromId and [AssignedToId]=@AssignedToId and [DivisionId]=@DivisionId and [OilTypeId]=@OilTypeId and [MonthId]=@MonthId and [FinancialYearId]=@FinancialYearId and [Year]=@Year
		END
		SELECT @SalesOrganizationCode as SalesOrganizationCode,@DistributionChannelCode as DistributionChannelCode,@AssignedFromUserCode as AssignedFromUserCode,@AssignedToUserCode as AssignedToUserCode,@Month as 'Month',@FinancialYear as FinancialYear,@Quarter as 'Quarter',@DivisionCode as DivisionCode,@OilTypeName as OilTypeName,@Year as 'Year',@Target as 'Target', 'Record Updated' as 'Message'  FROM [UserCustomerSaudaTargets] Where [AssignedFromId]= @AssignedFromId and [AssignedToId]=@AssignedToId and [DivisionId]=@DivisionId and [OilTypeId]=@OilTypeId and [MonthId]=@MonthId and [FinancialYearId]=@FinancialYearId and [Year]=@Year
...
--	SELECT @AssignedFromUserCode as AssignedFromUserCode,@AssignedToUserCode as AssignedToUserCode,@Month as 'Month',@FinancialYear as FinancialYear,@Quarter as 'Quarter',@DivisionCode as DivisionCode,@OilTypeName as OilTypeName,@Year as 'Year',@Target as 'Target', 'Record Updated' as 'Message' FROM [UserCustomerSaudaTargets] Where [AssignedFromId]= @AssignedFromId and [AssignedToId]=@AssignedToId
--END
...
/****** Object:  StoredProcedure [dbo].[SetUserDepotMapping]    Script Date: 15-11-2018 04:34:41 PM ******/
SET ANSI_NULLS ON
GO
